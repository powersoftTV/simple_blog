	...
	$.ajax({ 
		...
		{ 
			...
			jQuery.each(myArray,function(i,val) {
				...
				var j = i+1;
				result = j % num;
				//if(result == 0 && i != 0){$('#111').append('<tr>'); alert('<tr>');};	
				$('#111').append('<td style="vertical-align:top;"><div id="'+i+'"><table align="center" class="table-striped table-bordered  table-condensed"><tr><td width="215" height="215" align="center"><img title="'+val+'" src="uploader/uploads/'+ data.id +"/tumb/" + val + '?a='+ankap+'" alt="' + val + '?a='+ankap+'"></td><td  height="215" align="left"><a href="javascript:void(0)">'+data.filenameshort[i]+'<br />'+data.filex[i]+' x '+data.filey[i]+'<br />'+data.filedate[i]+'<br />'+data.filesize[i]+' kb<br /></a><a href="javascript:void(0)" onClick="del(\''+ val + '\','+i+')" >удалить</a></td></tr></table></div></td>');
				if(result == 0 && i != 0 && j != length){$('#111').append('</tr><tr>')};
			} );
			$('#111').append('</tr></table>');
