...
function fulltable(){
	...
	$.ajax({ 
		...
		jQuery.each(data.files,function(i,val) {
    		...
    		var num = 4;
			result = i % 3;
			if(result == 0 && i != 0){$('#111').append('<tr>')}	
		$('#111').append('<td><div id="'+i+'"><table class="table table-striped table-bordered  table-condensed"><tr><td><img width="200px" src="uploader/uploads/'+ data.id +"/tumb/" + val + '?a='+ankap+'" alt="' + val + '?a='+ankap+'"></td><td width="150px"><a href="javascript:void(0)" onClick="rot(\''+ val + '\',\'left\','+false+','+i+')">повернуть налево</a><br /><a href="javascript:void(0)" onClick="rot(\''+ val + '\',\'rigth\','+false+','+i+')">повернуть направо</a><br /><a href="javascript:void(0)" onClick="edit(\''+ val + '\','+false+','+i+')">миниатюра</a><br /><a href="javascript:void(0)" onClick="del(\''+ val + '\','+i+')" >удалить</a><br/><a href="javascript:void(0)" onClick="makemain(\''+ val + '\','+i+')" >сделать главной</a></td></tr></table></div></td>');  
